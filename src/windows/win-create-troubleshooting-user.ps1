...
Param(
    ...
    [Parameter(Mandatory = $false)][ValidatePattern("^.{12,123}$")][ValidateScript({ $notReserved = $_ -notin @('password', 'pa$$word', 'pa$$w0rd', 'pa$$word123', 'pa$$w0rd123', 'password123', '123456', 'admin', 'administrator', 'admin123', 'letmein', 'welcome', 'qwerty', 'abc123', 'abc@123', 'monkey', '123123', 'password1', 'adminadmin', 'sunshine', 'master', 'hannah', 'qazwsx', 'charlie', 'superman', 'iloveyou', 'princess', 'adminadmin123', 'login', 'admin1234', 'welcome123', 'adminadminadmin', 'adminadminadmin123', 'Password!', 'Password1', 'Password22', 'iloveyou!');  $lowercaseMatch = $_ -cmatch "[a-z]+";  $uppercaseMatch = $_ -cmatch "[A-Z]+";  $digitMatch = $_ -match "[0-9]+";  $specialCharMatch = $_ -match "[ `~!@#$%^&*()=+_\[\]{}\|:.\/'"",<>?]+";  $conditionsMet = ($lowercaseMatch, $uppercaseMatch, $digitMatch, $specialCharMatch) | Measure-Object -Sum | Select-Object -ExpandProperty Sum; return $notReserved  -and ($conditionsMet -gt 2) })][string]$password
)
